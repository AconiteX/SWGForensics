...
SharedWaypointObjectTemplate::SharedWaypointObjectTemplate(IffStream* str) :
...
void SharedWaypointObjectTemplate::parseFileData(IffStream* iffStream) {
...
void SharedWaypointObjectTemplate::parseVariableData(const QString& varName, Chunk* data) {
}

void SharedWaypointObjectTemplate::toLua(QTextStream& buffer) {
	//writeLuaHeader(buffer);

	buffer << endl;

	buffer << luaObjectName << " = SharedWaypointObjectTemplate:new {" << endl;
