...
SharedIntangibleObjectTemplate::SharedIntangibleObjectTemplate(IffStream* str) :
...
void SharedIntangibleObjectTemplate::parseFileData(IffStream* iffStream) {
...
void SharedIntangibleObjectTemplate::parseVariableData(const QString& varName, Chunk* data) {

}

void SharedIntangibleObjectTemplate::toLua(QTextStream& buffer) {
	//writeLuaHeader(buffer);

	buffer << endl;

	buffer << luaObjectName << " = SharedIntangibleObjectTemplate:new {" << endl;
